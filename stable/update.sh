...
while read chart ; do
  if ! $git diff-index HEAD --quiet $chart ; then
    ...
    $git --no-pager diff  --color=always -U0 --word-diff $chart/Chart.yaml | $_sed '1,/@@/d' | { grep --color=never '^version' || true ; }
    ...
    if $git diff-index HEAD --quiet $chart/Chart.yaml ; then
      ...
      $_sed -i "s/version:.*/version: $new_version/" $charts_home/$chart/Chart.yaml
    else
      :
    fi
  else 
...
while read chart ; do
  if ! $git diff-index HEAD --quiet $chart/requirements.yaml ; then
    ...
    $git --no-pager diff  --color=always -U1 --word-diff -z $charts_home/$chart/requirements.yaml | $_sed '1,/@@/d' | tr  '\n' ','
    if $git diff-index HEAD --quiet $charts_home/$chart/Chart.yaml ; then
      echo " But the version of $chart did not changed!"
  ...
  while read dependency ; do
    IFS='=' read -r -a old_versions <<< $($_sed -n "/name: $dependency/,/  version:/ {
          s/\s*alias:\s*\(.*\)/\1/p
          s/\s*version:\s*\(.*\)/\1=/p
    ...
    for pair in "${old_versions[@]}" ; do
      ...
      if [[ "$old_version" != "$current_version" ]]; then
       ...
       if [[ "$alias" != "" ]] ; then
        ...
        $_sed -i "/name: $dependency/,/  version:/ {
              /alias: $alias/,/  version:/ {
  ...
  done < <($_sed  -n "s/\s*-\s*name:\s*\(.*\)/\1/p" $charts_home/$chart/requirements.yaml | uniq)
...
a() {
  ...
  while read dependency ; do
    ...
    if [[ "$dependent_chart" == "$dependency" ]]; then
      old_version=$($_sed  -n "/name: $dependency/,+1 {
        s/\s*version:\s*\(.*\)/\1/p
      ...
      if [[ "$old_version" != "$dep_version" ]]; then
        ...
        $_sed -n "/name: $1/{N;s/\(version: $old_version\).*/\1$dep_version/p}" "$chart/requirements.yaml"
        to_upgrade=1
      else
      ...
      (( indent-=3 ))
      if (( $to_upgrade )); then
        old_v=$($_sed  -n "s/\.*version:\s*\(.*\)/\1/p" $dependency/Chart.yaml)
        local last
        last=${old_v##*.}
        (( last++ ))
        new_v="${old_v%.*}.$last"
        #$_sed -n "s/\(\.*version:\s*\).*/\1${new_v}/p" $lrequirements/Chart.yaml
  ...
  done < <($_sed  -n "s/\s*-\s*name:\s*\(.*\)/\1/p" $chart/requirements.yaml | uniq)
}
...
$git -C "$TMP_DIR" status
$git -C "$TMP_DIR" commit -m "relea$_sed chart $chart $version"
$git -C "$TMP_DIR" push
